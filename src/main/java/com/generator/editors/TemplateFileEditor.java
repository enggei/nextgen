...
public class TemplateFileEditor extends JPanel {
	...
	public TemplateFileEditor() {
		...
		txtEditor.setFont(new Font("Hack", Font.PLAIN, 13));
...
//					component.setIcon(IconManager.getIcon(entity.getType().name()));
	...
	private void saveModel() {
		...
		TemplateVisitor.visit(currentTemplateFile.getFile(),
			new TemplateGroupGenerator(txtRoot.getText(), builderPackage),
			new TemplateNeoGenerator(txtRoot.getText(), builderPackage),
			new TemplatesVertxGenerator(txtRoot.getText(), builderPackage),
			new TemplateSwingGenerator(txtRoot.getText(), builderPackage));
