...
public abstract class NeoEditor extends BaseEditor<NeoPNode, NeoRelationshipPath> implements TransactionEventHandler<Object> {
	...
	protected Node lastLayoutSaved = null;
	protected final Map<Long, UUID> neoRelationPaths = new LinkedHashMap<>();
...
//	protected final AtomicBoolean isExpanding = new AtomicBoolean(false);
	...
	public Object beforeCommit(TransactionData transactionData) throws Exception {
		...
		transactionData.deletedNodes().forEach(new Consumer<Node>() {
			@Override
			public void accept(Node node) {
				deletedNodes.add(node.getId());
			}
		});

		transactionData.deletedRelationships().forEach(relationship -> deletedRelations.add(relationship.getId()));

		transactionData.createdRelationships().forEach(new Consumer<Relationship>() {
			@Override
	...
	public void afterCommit(TransactionData transactionData, Object o) {
		...
		deletedNodes.forEach(new Consumer<Long>() {
			@Override
		...
		deletedRelations.forEach(new Consumer<Long>() {
			@Override
			public void accept(Long relationshipId) {
		...
		addedRelations.forEach(new BiConsumer<Long, Map<String, UUID>>() {
			@Override
			public void accept(Long aLong, Map<String, UUID> stringUUIDMap) {
				System.out.println("relation " + aLong + " " + stringUUIDMap.get("src") + " -> " + stringUUIDMap.get("dst"));
			}
		});
	...
	public Action showAllNodesByLabel(final Label label, PInputEvent event) {
		return new TransactionAction("Show all " + label, graph, canvas) {
			...
			public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
				...
				graph.findNodes(label).forEachRemaining(new Consumer<Node>() {
					@Override
					public void accept(Node node) {
	...
	public Action saveLayout(final Node node, final String name) {
		return saveLayout(null, node, name);
	}

	public Action saveLayout(final String actionName, final Node node, final String name) {
		return new TransactionAction(actionName == null ? name : actionName, graph, canvas) {
			@Override
			public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {

				// delete old members
				outgoing(node, layoutMember).forEach(new Consumer<Relationship>() {
					@Override
					public void accept(Relationship relationship) {
						relationship.delete();
					}
	...
	public Action loadLayout(final Node node, final String name) {
	...
	public Action deleteLayout(final Node node, final String name) {
		return new TransactionAction(name, graph, canvas) {
			@Override
			public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
				outgoing(node, layoutMember).forEach(new Consumer<Relationship>() {
