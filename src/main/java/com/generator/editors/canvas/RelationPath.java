...
public class RelationPath<S extends BasePNode, T extends BasePNode> implements PropertyChangeListener, Comparable<RelationPath<S, T>> {
   ...
   private PaintRelationStatus showLabel = PaintRelationStatus.showLines;
   ...
   private void drawPath(Point2D start, Point2D ctr1, Point2D ctr2, Point2D end) {
      ...
      path.reset();
      if (PaintRelationStatus.showNothing.equals(showLabel)) return;
...
//		path.drawLine(0, 0, len, 0);
   ...
   public final void showLabel(PaintRelationStatus status) {

      if (this.showLabel.equals(status)) return;
      this.showLabel = status;

      switch (this.showLabel) {
