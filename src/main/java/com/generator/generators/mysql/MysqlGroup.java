...
public final class MysqlGroup {
   ...
   public final class selectST implements MysqlGroupTemplate {
	...
	">>\n" + 
	"\n" + 
	"createColumn(autoIncrement,comment,defaultValue,name,nullable,onUpdate,type) ::= <<`~name~` ~type~~if(nullable)~~else~ NOT NULL~endif~~if(autoIncrement)~ auto_increment~endif~~if(defaultValue)~ default ~defaultValue~~endif~~if(onUpdate)~ on update ~onUpdate~~endif~~if(comment)~ COMMENT '~comment~'~endif~\n" + 
	">>\n" + 
	"\n" + 
	...
	">>\n" + 
	"\n" + 
	"createKey(key_,indexType,name) ::= <<KEY ~if(name)~`~name~` ~endif~~if(indexType)~USING ~indexType~ ~endif~(~key_:{it|`~it~`}; separator=\",\"~)\n" + 
	">>\n" + 
	"\n" + 
	...
	">>\n" + 
	"\n" + 
	"alterTableUpdateForeignKeyConstraint(refTable,column,name,onDelete,onUpdate,refColumn,table) ::= <<ALTER TABLE `~table~` DROP FOREIGN KEY `~name~`, ADD CONSTRAINT `~name~` FOREIGN KEY (`~column~`) REFERENCES `~refTable~` (`~refColumn~`) ON DELETE ~if(onDelete)~~onDelete~~else~RESTRICT~endif~ ON UPDATE ~if(onUpdate)~~onUpdate~~else~RESTRICT~endif~;\n" + 
	">>\n" + 
	"\n" + 
	"join(tables,columns,joins,order) ::= <<SELECT ~columns:{it|~it~};separator=\", \"~\n" + 
	...
	">>\n" + 
	"\n" + 
	"insert(table,values,columns) ::= <<INSERT INTO TABLE ~table~ (~columns:{it|~it~};separator=\",\"~) VALUES (~values:{it|~it~};separator=\",\"~);\n" + 
	">>\n" + 
	"\n" + 
	"alterTableAddUniqueKey(table,columns,indexType,name) ::= <<ALTER TABLE `~table~` ADD UNIQUE INDEX `~name~` ~if(indexType)~USING ~indexType~ ~endif~(~columns:{it|`~it~`}; separator=\", \"~);\n" + 
	">>\n" + 
	"\n" + 
	...
	">>\n" + 
	"\n" + 
	"alterTableUpdateUniqueKey(name,table,columns,indexType) ::= <<ALTER TABLE `~table~` DROP INDEX `~name~`, ADD UNIQUE INDEX `~name~` ~if(indexType)~USING ~indexType~ ~endif~(~columns:{it|`~it~`}; separator=\", \"~);\n" + 
	">>\n" + 
	"\n" + 
	...
	">>\n" + 
	"\n" + 
	"alterTableAddIndex(name,table,columns,indexType) ::= <<ALTER TABLE `~table~` ADD INDEX `~name~` ~if(indexType)~USING ~indexType~ ~endif~(~columns:{it|`~it~`}; separator=\", \"~);\n" + 
	">>\n" + 
	"\n" + 
	"select(table,columns,order) ::= <<SELECT ~columns:{it|~it~};separator=\", \"~ FROM ~table~~if(order)~ ORDER BY ~order:{it|~it~};separator=\",\"~~endif~;\n" + 
	">>\n" + 
	"\n" + 
	"";
}