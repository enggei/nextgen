 ...
 * goe on 4/23/15.
 */
public class EasyFlowTemplateGenerator extends EasyFlowDomainVisitor {

	private final EasyFlowGroup group = new EasyFlowGroup();

	private EasyFlowGroup.easyFlowST fsm;

	public EasyFlowTemplateGenerator(Component component) {
		super(component, "Generate Stringtemplate for fsm");
	}

	@Override
	<T> T visitFlow(Node node) {

		// todo: nb: see replacement-group in constructor
//        setTargetFile(node, "targetFile");
//
		...
		final String contextName = getString(node, "contextName");
