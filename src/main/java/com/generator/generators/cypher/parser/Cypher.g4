...
readOnlyEnd : readPart cypher_return ;

readUpdateEnd : readingClause ( SP? readingClause )* ( SP? updatingClause )+ ( SP? cypher_return )? ;

updatingEnd : updatingStartClause ( SP? updatingClause )* ( SP? cypher_return )? ;
...
readingClause : cyper_match
              | unwind
              | inQueryCall
              ;

cyper_match : ( OPTIONAL SP )? MATCH SP? pattern ( SP? where )? ;
...
cypher_return : RETURN ( SP? DISTINCT )? SP returnBody ;
