...
public class TemplateDomain {
   ...
   public static NeoPNode newPNode(Node node, String nodetype, NeoEditor neoEditor) {
      switch (Entities.valueOf(nodetype)) {
         ...
         case SingleTemplateParameter:
            return new TemplateParameterPNode(node, SingleTemplateParameter, Properties.name.name(), neoEditor);
         case ListTemplateParameter:
            return new TemplateParameterPNode(node, ListTemplateParameter, Properties.name.name(), neoEditor);
         case KeyValueTemplateParameter:
            return new TemplateParameterPNode(node, KeyValueTemplateParameter, Properties.name.name(), neoEditor);
         case Statement:
         ...
         case KeyValueSet:
            return new KeyValueSetPNode(node, new CompositePText(), neoEditor, other(node, singleOutgoing(node, TEMPLATE_PARAMETER)));
         case Project:
   ...
   static Node newDirectory(NeoModel db, String path) {
      ...
      node.setProperty(Properties.name.name(), path);
      return node;
   }
...
//		newTemplateStatement(db, node, "bugFixEndStatement", ">");
   ...
   public static void addToMenu(JPopupMenu pop, PInputEvent event, NeoEditor editor) {
      final JMenu newMenu = new JMenu("Templates");
      newMenu.add(new NewProject(event,editor));
      newMenu.add(new NewTestGroup(event,editor));
      newMenu.add(new NewTemplateGroup(event,editor));
      newMenu.add(new ParseTemplateGroup(event,editor));
      pop.add(newMenu);
   }
   ...
   static void renderProjectMember(final Node node, final JComponent component) {
      outgoing(node, DIRECTORY_MEMBER).forEach(projectRelation -> {
         ...
         final String root = getString(node, Properties.name.name());
         final String outputFormat = getString(projectRelation, Properties.outputFormat.name());
         ...
         switch (outputFormat) {
            ...
            case "java": {
               ...
               final String packageParameter = getString(projectRelation, Properties.packageName.name());
               final String nameParameter = getString(projectRelation, Properties.name.name());
            ...
            case "other": {
               ...
               final String filename = getString(projectRelation, Properties.name.name());
   ...
   static String renderReferenceNode(Node referenceNode, Entities referenceNodeType) {
   ...
   static Relationship addNodeReference(Node referencedNode, Node node, RelationshipType type) throws NeoEditor.CircularStatementException {
      ...
      if (referencedNode.hasLabel(Statement) || referencedNode.hasLabel(SingleValue)) {
         ...
         newRelationship.setProperty(Properties.relationType.name(), referencedNode.hasLabel(Statement) ? Statement.name() : SingleValue.name());
   ...
   private static NeoPNode parseTemplateGroupFile(File file, PInputEvent event, NeoEditor editor) {
      ...
      final Iterator<Node> iterator = editor.getGraph().getAll(TemplateGroup.name(), TemplateDomain.Properties.name.name(), name).iterator();
      if (iterator.hasNext()) {
      ...
      final Node templateGroup = TemplateDomain.newTemplateGroup(editor.getGraph(), name, templateFile.getDelimiter() + "");
      ...
      for (com.generator.generators.templates.domain.TemplateStatement templateStatement : templateFile.getStatements())
         TemplateDomain.importTemplateStatement(templateGroup, templateStatement, editor);
   ...
   private static class NewProject extends NeoEditor.TransactionAction {

      private final PInputEvent event;

      NewProject(PInputEvent event, NeoEditor editor) {
         super("New Project", editor);
         this.event = event;
      }
      ...
      public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
         ...
         editor.show(uuidOf(TemplateDomain.newProject(graph, name)), Project.name()).
               setOffset(event);
      }
   }
   ...
   private static class NewTemplateGroup extends NeoEditor.TransactionAction {
      ...
      public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
         ...
         editor.show(uuidOf(TemplateDomain.newTemplateGroup(graph, name.split(" ")[0], name.split(" ")[1])), TemplateGroup.name()).
               setOffset(event);
      }
   }
      ...
      TemplateEditor(String delimiter, String name, String text, String initLabel) {
         ...
         txtEditor.addKeyListener(new KeyAdapter() {
            ...
            private void insertListProperty() {
               ...
               final String input = SwingUtil.showInputDialog(TemplateDomain.Properties.name.name(), txtEditor);
   ...
   static class TemplateDomainPNode extends NeoPNode<PText> {
      ...
      TemplateDomainPNode(Node node, PText representation, TemplateDomain.Entities nodeType, String[] defaultColor, NeoEditor editor) {
   ...
   static class DirectoryPNode extends TemplateDomainPNode {

      DirectoryPNode(Node node, NeoEditor editor) {
         super(node, Directory, TemplateDomain.Properties.name.name(), "153, 52, 4".split(", "), editor);
         pNode.setFont(new Font("Hack", Font.BOLD, 12));
      }
      ...
      public void expand() {
         ...
         outgoing(node, DIRECTORY_MEMBER).forEach(relationship -> pNodes.put(uuidOf(other(node, relationship)), TemplateDomain.Entities.Statement));
      ...
      public void showDependents() {
         ...
         for (Relationship relationship : node.getRelationships(INCOMING)) {
            ...
            final Node other = other(node, relationship);
            if (hasLabel(other, TemplateDomain.Entities.Project.name()))
               pNodes.put(uuidOf(other(node, relationship)), TemplateDomain.Entities.Project);
            else
      ...
      @Override
      public void showNodeActions(JPopupMenu pop, PInputEvent event) {
         pop.add(new Render(editor));
         pop.add(new CopyPath(editor));
         pop.add(new SetPath(editor));

         outgoing(node, DIRECTORY_MEMBER).forEach(relationship -> {
            final Node statementNode = other(node, relationship);
            pop.add(new NeoEditor.TransactionAction("Detach " + NeoModel.getNameOrLabelFrom(statementNode), editor) {
               @Override
               public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
                  relationship.delete();
               }
      ...
      private static Relationship setRenderOtherFile(String filename, Relationship newRelation) {
         newRelation.setProperty(TemplateDomain.Properties.outputFormat.name(), "other");
         newRelation.setProperty(TemplateDomain.Properties.name.name(), filename);
         return newRelation;
      }

      private static Relationship setRenderJavaFile(String packageParam, String classnameParam, Relationship newRelation) {
         newRelation.setProperty(TemplateDomain.Properties.outputFormat.name(), "java");
         newRelation.setProperty(TemplateDomain.Properties.packageName.name(), packageParam);
         newRelation.setProperty(TemplateDomain.Properties.name.name(), classnameParam);
         return newRelation;
      }

      private class SetPath extends NeoEditor.TransactionAction {

         SetPath(NeoEditor editor) {
            super("Set path ", editor);
         }

         @Override
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {

            final File dir = SwingUtil.showOpenDir(editor.canvas, getString(node, TemplateDomain.Properties.name.name()));
            if (dir == null) return;

            node.setProperty(TemplateDomain.Properties.name.name(), dir.getAbsolutePath());
      ...
      private class CopyPath extends NeoEditor.TransactionAction {
         ...
         CopyPath(NeoEditor editor) {
            super("Copy path ", editor);
         }

         @Override
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            SwingUtil.toClipboard(getString(node, TemplateDomain.Properties.name.name()));
         }
      }

      private class Render extends NeoEditor.TransactionAction {

         public Render(NeoEditor editor) {
            super("Render", editor);
         }
   ...
   static class KeyValueSetPNode extends TemplateDomainPNode {
      ...
      public void expand() {
         final Map<UUID, org.neo4j.graphdb.Label> pNodes = new LinkedHashMap<>();
         for (String key : getString(templateParameter, TemplateDomain.Properties.keys.name()).split(" ")) {
            final Relationship kvRelation = singleOutgoing(node, RelationshipType.withName(key));
            if (kvRelation == null) continue;
            pNodes.put(uuidOf(other(node, kvRelation)), TemplateDomain.Entities.valueOf(getString(kvRelation, TemplateDomain.Properties.relationType.name())));
         }
         editor.showAndLayout(pNodes, pNode);
      }
      ...
      public void updateView() {
         ...
         for (String key : getString(templateParameter, TemplateDomain.Properties.keys.name()).split(" ")) {
            ...
            keyName.setText(TemplateDomain.renderReferenceNode(other(node, kvRelation), TemplateDomain.Entities.valueOf(getString(kvRelation, TemplateDomain.Properties.relationType.name()))));
         }
      }
      ...
      public void showNodeActions(JPopupMenu pop, PInputEvent event) {
         ...
         final String[] keys = getString(templateParameter, TemplateDomain.Properties.keys.name()).split(" ");
         for (String key : keys) {
      ...
      private class Edit extends NeoEditor.TransactionAction {
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            for (String key : keys) {
               ...
               final Node kvValue = other(node, kvRelation);
               final String textValue = TemplateDomain.renderReferenceNode(kvValue, TemplateDomain.Entities.valueOf(getString(kvRelation, TemplateDomain.Properties.relationType.name())));
   ...
   static class ProjectPNode extends TemplateDomainPNode {

      private String lastDir = System.getProperty("user.home");

      ProjectPNode(Node node, NeoEditor editor) {
         super(node, Project, TemplateDomain.Properties.name.name(), "153, 52, 4".split(", "), editor);
      }
      ...
      public void expand() {
         ...
         outgoing(node, PROJECT_DIRECTORY).forEach(relationship -> pNodes.put(uuidOf(other(node, relationship)), TemplateDomain.Entities.Directory));
      ...
      @Override
      public void showNodeActions(JPopupMenu pop, PInputEvent event) {
         pop.add(new SetName(TemplateDomain.Properties.name.name(),editor));
         pop.add(new CreateMetaDomain(event, editor));
         pop.add(new AddDirectory(event, editor));
         super.showNodeActions(pop, event);
      }

      private class SetName extends NeoEditor.TransactionAction {

         SetName(String name, NeoEditor editor) {
      ...
      private class AddDirectory extends NeoEditor.TransactionAction {
         ...
         AddDirectory(PInputEvent event, NeoEditor editor) {
            super("Add Directory", editor);
            this.event = event;
         }
      ...
      private class CreateMetaDomain extends NeoEditor.TransactionAction {
         ...
         CreateMetaDomain(PInputEvent event, NeoEditor editor ) {
            super("Create Meta Domain", editor);
            this.event = event;
         }
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            editor.show(uuidOf(project), MetaDomain.Entities.DOMAIN.name()).
                  setOffset(event);
   ...
   static class SingleValuePNode extends TemplateDomainPNode {
      ...
      public void showDependents() {
         ...
         for (Relationship relationship : node.getRelationships(INCOMING)) {
            if (NeoEditor.isAppRelated(relationship)) continue;
            final Node other = other(node, relationship);
            if (hasLabel(other, TemplateDomain.Entities.Statement.name()))
               pNodes.put(uuidOf(other(node, relationship)), TemplateDomain.Entities.Statement);
            else if (hasLabel(other, TemplateDomain.Entities.KeyValueSet.name()))
               pNodes.put(uuidOf(other(node, relationship)), TemplateDomain.Entities.KeyValueSet);
            else
               System.out.println(other.getLabels().iterator().next());
         }
      ...
      @Override
      public void updateView() {
         final String value = getString(node, TemplateDomain.Properties.value.name());
      ...
      private class SetValue extends NeoEditor.TransactionAction {
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            final String defaultValue = get(node, TemplateDomain.Properties.value.name());
            ...
            node.setProperty(TemplateDomain.Properties.value.name(), newValue.trim());
   ...
   static class StatementPNode extends NeoPNode<PNode> {
      ...
      StatementPNode(Node node, Node templateStatement, NeoEditor editor) {
         ...
         // getString(templateStatement, TemplateDomain.Properties.name.name())
      ...
      public void updateView() {
         ...
         final String property = getString(templateStatement, TemplateDomain.Properties.statementLabel.name());
         if (property == null) {
            nodeName.setText(getString(templateStatement, TemplateDomain.Properties.name.name()));
            return;
         }
      ...
      public void expand() {
         ...
         new StatementVisitor() {
            ...
            @Override
            protected void onListValue(String name, Node referenceNode, TemplateDomain.Entities referenceNodeType) {
               pNodes.put(uuidOf(referenceNode), referenceNodeType);
            }
      ...
      public void showDependents() {
         ...
         for (Relationship relationship : node.getRelationships(INCOMING)) {
            if (NeoEditor.isAppRelated(relationship)) continue;
            final Node other = other(node, relationship);
            if (hasLabel(other, TemplateDomain.Entities.Directory.name()))
               pNodes.put(uuidOf(other(node, relationship)), TemplateDomain.Entities.Directory);
            else
               System.out.println(other.getLabels().iterator().next());
         }
         ...
         pNodes.put(uuidOf(other(node, relationship)), TemplateDomain.Entities.TemplateStatement);
      ...
      public void showNodeActions(JPopupMenu pop, PInputEvent event) {
         ...
         new StatementVisitor() {
            ...
            @Override
            protected void onSingleValue(String name, Node referenceNode, TemplateDomain.Entities referenceNodeType) {
               removeMenu.add(new DetachReference(name, referenceNode, referenceNodeType, editor));
            }
      ...
      public void showTargetActions(JPopupMenu pop, PInputEvent event) {
         ...
         new TemplateGroupVisitor() {
            ...
            protected void onSingleTemplateParameter(String name, Node templateParameter) {
               ...
               if (statementParameterRelation != null) {
                  ...
                  if (templateStatementNode.equals(selectedTemplateStatementNode))
                     pop.add(new SetSingleReference(name, templateParameter, selectedNode, editor));
                  return;
               }
            ...
            protected void onListTemplateParameter(String name, Node templateParameter) {
               final Node selectedNode = selectedNodes.iterator().next().node;
               if (selectedNode.hasLabel(Statement) || selectedNode.hasLabel(SingleValue))
                  pop.add(new AddListReference(name, templateParameter, selectedNode, editor));
            }
         }.visitTemplateStatement(templateStatement);
      }
      ...
      private class SetSingleValue extends NeoEditor.TransactionAction {
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            final Node newSingleValue = TemplateDomain.newSingleValue(editor.getGraph(), newValue);
            TemplateDomain.setSingleReference(newSingleValue, node, RelationshipType.withName(get(templateParameter, TemplateDomain.Properties.name.name())), editor);
      ...
      private class AddListValues extends NeoEditor.TransactionAction {
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            for (String newValue : newValues.split(" ")) {
               ...
               TemplateDomain.addNodeReference(newListValue, node, RelationshipType.withName(get(templateParameter, TemplateDomain.Properties.name.name())));
               // no need for editor.addRelation, as show-method will ensure its visible
      ...
      private class DetachReference extends NeoEditor.TransactionAction {
         ...
         DetachReference(String name, Node referencedNode, TemplateDomain.Entities nodeType, NeoEditor editor) {
            super("Remove " + name + " : " + TemplateDomain.renderReferenceNode(referencedNode, nodeType), editor);
            this.referencedNode = referencedNode;
         }
      ...
      private class AddKeyValueSets extends NeoEditor.TransactionAction {
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            contentPanel.add(new JLabel("Add " + getString(templateParameter, TemplateDomain.Properties.name.name())));
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            new StatementVisitor() {
               ...
               protected void onSingleValue(String name, Node referenceNode, TemplateDomain.Entities referenceNodeType) {
                  ...
                  if (TemplateDomain.Entities.Statement.equals(referenceNodeType)) return;
            ...
            SwingUtil.showDialogNoDefaultButton(form, editor.canvas, "Statement", () -> editor.doInTransaction(tx1 -> {
               for (String singleValueName : singleValues.keySet()) {
                  ...
                  if (!newValue.equals(existingValues.get(singleValueName))) {
                     ...
                     final Node newSingleValue = TemplateDomain.newSingleValue(editor.getGraph(), newValue);
                     TemplateDomain.setSingleReference(newSingleValue, node, RelationshipType.withName(get(templateParameters.get(singleValueName), TemplateDomain.Properties.name.name())), editor);
                     // no need for editor.addRelation, as show-method will ensure its visible
                     editor.
                           show(uuidOf(newSingleValue), SingleValue.name()).
                           setOffset(event);
      ...
      private class SetSingleReference extends NeoEditor.TransactionAction {
         ...
         SetSingleReference(String name, Node templateParameter, Node selectedNode, NeoEditor editor) {
            super("Set " + name, editor);
            this.templateParameter = templateParameter;
         ...
         @Override
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            editor.addRelation(TemplateDomain.setSingleReference(selectedNode, node, RelationshipType.withName(get(templateParameter, TemplateDomain.Properties.name.name())), editor));
            updateView();
         }
      }
      ...
      private class AddListReference extends NeoEditor.TransactionAction {
         ...
         AddListReference(String name, Node templateParameter, Node selectedNode, NeoEditor editor) {
         ...
         @Override
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            editor.addRelation(TemplateDomain.addNodeReference(selectedNode, node, RelationshipType.withName(get(templateParameter, TemplateDomain.Properties.name.name()))));
            editor.clearMousePosition();
         }
      }
   }
   ...
   static public class StatementVisitor {
      ...
      final void visitStatement(Node statement) {
         ...
         for (Relationship templateParameterRelation : incoming(statementTemplate, TEMPLATE_PARAMETER)) {
            ...
            final RelationshipType parameterName = RelationshipType.withName(get(templateParameter, TemplateDomain.Properties.name.name()));

            switch (TemplateDomain.Entities.valueOf(getString(templateParameterRelation, TemplateDomain.Properties.relationType.name()))) {
               ...
               case SingleTemplateParameter: {
                  ...
                  onSingleValue(parameterName.name(), other(statement, relationship), TemplateDomain.Entities.valueOf(getString(relationship, TemplateDomain.Properties.relationType.name())));
                  break;
               }
               case ListTemplateParameter: {

                  for (Relationship relationship : outgoing(statement, parameterName))
                     onListValue(parameterName.name(), other(statement, relationship), TemplateDomain.Entities.valueOf(getString(relationship, TemplateDomain.Properties.relationType.name())));
                  break;
               }
               case KeyValueTemplateParameter: {
                  ...
                  for (Relationship relationship : outgoing(statement, parameterName)) {
                     ...
                     for (String key : getString(templateParameter, TemplateDomain.Properties.keys.name()).split(" ")) {
                        ...
                        onKeyValue(key, kvValue, TemplateDomain.Entities.valueOf(getString(kvRelation, TemplateDomain.Properties.relationType.name())));
                     }
      ...
      protected void onKeyValue(String name, Node referenceNode, TemplateDomain.Entities referenceNodeType) {
      }
   ...
   static class TemplateGroupPNode extends TemplateDomainPNode {

      TemplateGroupPNode(Node node, NeoEditor editor) {
         super(node, TemplateGroup, TemplateDomain.Properties.name.name(), "64, 64, 64".split(", "), editor);
      }
      ...
      private class NewTemplateStatement extends NeoEditor.TransactionAction {
         ...
         @Override
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            final String delimiter = get(node, TemplateDomain.Properties.delimiter.name());
            final TemplateDomain.TemplateEditor statementEditorPanel = new TemplateDomain.TemplateEditor(delimiter, "", "", null);
      ...
      private class CreateGroupFile extends NeoEditor.TransactionAction {
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            final String packageName = SwingUtil.showInputDialog("Package", editor.canvas, getString(node, TemplateDomain.Properties.packageName.name()));
            node.setProperty(TemplateDomain.Properties.packageName.name(), packageName);

            final String groupName = getString(node, TemplateDomain.Properties.name.name()) + "Group";
      ...
      private class ToSTGGroup extends NeoEditor.TransactionAction {
         ...
         ToSTGGroup(NeoEditor editor) {
            super("As STGroup", editor);
         }
      ...
      private class ExpandTemplateGroup extends NeoEditor.TransactionAction {
         ...
         ExpandTemplateGroup(NeoEditor editor) {
            super("Expand", editor);
         }
   ...
   static public class TemplateGroupVisitor {
      ...
      final void visitTemplateGroup(Node templateGroup) {
         ...
         onTemplateGroupStart(get(templateGroup, TemplateDomain.Properties.name.name()), get(templateGroup, TemplateDomain.Properties.delimiter.name()), templateGroup);
      ...
      final void visitTemplateStatement(Node templateStatement) {
         ...
         onTemplateStatementStart(get(templateStatement, TemplateDomain.Properties.name.name()).toString(), get(templateStatement, TemplateDomain.Properties.text.name()).toString(), templateStatement);
         ...
         for (Relationship parameterRelationship : incoming(templateStatement, TEMPLATE_PARAMETER)) {
            ...
            onTemplateParameterStart(getString(templateParameter, TemplateDomain.Properties.name.name()));
            switch (TemplateDomain.Entities.valueOf(getString(parameterRelationship, TemplateDomain.Properties.relationType.name()))) {

               case SingleTemplateParameter:
                  // use this to check if templateParameter is statement-parameter
                  //final Relationship statementParameterRelation = BaseDomainVisitor.singleOutgoing(templateParameter, STATEMENT_PARAMETER);
                  onSingleTemplateParameter(get(templateParameter, TemplateDomain.Properties.name.name()), templateParameter);
                  break;

               case ListTemplateParameter:
                  onListTemplateParameter(get(templateParameter, TemplateDomain.Properties.name.name()), templateParameter);
                  break;

               case KeyValueTemplateParameter:
                  onKeyValueTemplateParameter(get(templateParameter, TemplateDomain.Properties.name.name()), get(templateParameter, TemplateDomain.Properties.keys.name()), templateParameter);
                  break;
            }
            onTemplateParameterEnd(getString(templateParameter, TemplateDomain.Properties.name.name()));
         }

         onTemplateStatementEnd(get(templateStatement, TemplateDomain.Properties.name.name()).toString(), get(templateStatement, TemplateDomain.Properties.text.name()), templateStatement);
      }
   ...
   static class TemplateParameterPNode extends TemplateDomainPNode {

      TemplateParameterPNode(Node node, TemplateDomain.Entities nodeType, String property, NeoEditor editor) {
      ...
      public void expand() {
         final Map<UUID, Label> pNodes = new LinkedHashMap<>();
         incoming(node, TEMPLATE_PARAMETER).forEach(relationship -> pNodes.put(uuidOf(other(node, relationship)), TemplateDomain.Entities.valueOf(getString(relationship, TemplateDomain.Properties.relationType.name()))));
         editor.showAndLayout(pNodes, pNode);
      }
   }

   static class TemplateStatementPNode extends TemplateDomainPNode {

      TemplateStatementPNode(Node templateStatement, NeoEditor editor) {
         super(templateStatement, TemplateStatement, TemplateDomain.Properties.name.name(), "64, 64, 64".split(", "), editor);
      }
      ...
      public void renderTo(JTextComponent textArea) {
         editor.doInTransaction(tx -> {
            textArea.setText(get(node, TemplateDomain.Properties.text.name()));
            textArea.setCaretPosition(0);
         });
      }
      ...
      private class EditTemplateStatment extends NeoEditor.TransactionAction {
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            final String delimiter = get(templateGroup, TemplateDomain.Properties.delimiter.name());
            final String name = getString(node, TemplateDomain.Properties.name.name());
            final String text = getString(node, TemplateDomain.Properties.text.name());
            final String statementLabel = getString(node, TemplateDomain.Properties.statementLabel.name());

            final TemplateDomain.TemplateEditor statementEditorPanel = new TemplateDomain.TemplateEditor(delimiter, name, text, statementLabel);
                  ...
                  editor.getGraph().doInTransaction(new NeoModel.Committer() {
                     ...
                     public void doAction(Transaction tx) throws Throwable {
                        ...
                        for (Map.Entry<Node, String> kvEntry : updatedKeySets.entrySet())
                           kvEntry.getKey().setProperty(TemplateDomain.Properties.keys.name(), kvEntry.getValue());
                        ...
                        node.setProperty(TemplateDomain.Properties.name.name(), editedStatement.getName());
                        node.setProperty(TemplateDomain.Properties.text.name(), editedStatement.getText().trim());
                        ...
                        for (TemplateParameter parameter : editedStatement.getParameters()) {
                           if (parameter.getPropertyName().equals(statementEditorPanel.getStatementLabel())) {
                              ...
                              node.setProperty(TemplateDomain.Properties.statementLabel.name(), statementEditorPanel.getStatementLabel());
                              break;
                           }
                        }
                        if (!foundStatementLabelParameter)
                           node.removeProperty(TemplateDomain.Properties.statementLabel.name());
      ...
      private class SetTemplateStatementParameter extends NeoEditor.TransactionAction {
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            ...
            if (isRelated(parameterNode, templateStatement, TemplateDomain.Relations.STATEMENT_PARAMETER))
               getRelationship(parameterNode, templateStatement, TemplateDomain.Relations.STATEMENT_PARAMETER).delete();

            final Relationship statementParameterRelation = parameterNode.createRelationshipTo(templateStatement, TemplateDomain.Relations.STATEMENT_PARAMETER);
      ...
      private class NewStatement extends NeoEditor.TransactionAction {
         ...
         NewStatement(NeoEditor editor) {
            super("New " + get(node, TemplateDomain.Properties.name.name()), editor);
         }

         @Override
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            editor.
                  show(uuidOf(TemplateDomain.newStatement(editor.getGraph(), node)), TemplateDomain.Entities.Statement.name()).
      ...
      private class ShowTemplateGroup extends NeoEditor.TransactionAction {
         ...
         public void actionPerformed(ActionEvent e, Transaction tx) throws Exception {
            editor.
                  show(uuidOf(other(node, singleOutgoing(node, TEMPLATE_GROUP))), TemplateDomain.Entities.TemplateGroup.name()).
                  setOffset(getRandomPosition());
         }
      }
