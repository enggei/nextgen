...
public class TemplateFileParser {
    ...
    public TemplateStatement parse(String delimiters, String name, String content) throws IOException {
        return parse(delimiters, name, content, false);
    }
    ...
    public TemplateFile parse(File file, boolean debug) {
        this.debug = debug;
...
//      printErrors(runtimeErrors, "runtime error: ");
        ...
        for (STGroup stGroup : group.getImportedGroups()) {
            imports.put(stGroup.getName(), new TemplateFileParser().parse(new File(file.getParent(), stGroup.getName() + (stGroup.getName().toLowerCase().endsWith(".stg") ? "" : ".stg"))));
