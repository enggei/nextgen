...
public class Parameter {
	...
	static final String st = "Parameter(name,isVarargs,modifiers,type,annotations) ::= <<~annotations:{it|~it~};separator=\" \"~~if(annotations)~ ~endif~~modifiers:{it|~it~};separator=\" \"~~if(modifiers)~ ~endif~~if(type)~~type~ ~endif~~if(isVarargs)~... ~endif~~name~ >>";
}  