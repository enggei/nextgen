...
public class KVAccessors {
	...
	public KVAccessors setKeys(java.util.Collection<KVAccessors_Keys> values) {
			this._keys.clear();
			values.stream().map(KVAccessors_Keys::asMap).forEach(map -> _keys.add(map));
			return this;
		}
	...
	public java.util.List<Object> getKeys_Name() {
		return streamKeys().map(KVAccessors_Keys::getName).collect(java.util.stream.Collectors.toList());
	}
	...
	static final String st = "KVAccessors(keys,entity,name) ::= <<public ~entity;format=\"capitalize\"~ add~name;format=\"capitalize\"~(~keys:{it|String _~it.name~};separator=\", \"~) {\n" + 
