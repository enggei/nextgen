...
public class GradleST {
	...
	private static org.stringtemplate.v4.STGroup stGroup = decorate(new org.stringtemplate.v4.STGroupString("GradleST", stg, '~', '~'));

	public static void setSTGroup(final String stgFile) {
		stGroup = decorate(new org.stringtemplate.v4.STGroupFile(stgFile, '~', '~'));
	}

	public static Init newInit() {
		return new Init(stGroup);
	}  

	public static Install newInstall() {
		return new Install(stGroup);
	}  
