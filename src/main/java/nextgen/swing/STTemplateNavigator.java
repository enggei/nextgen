...
public class STTemplateNavigator extends JPanel {
	...
	public void onNewSTAction(nextgen.events.NewSTAction event) {
		findAllSTGroupTreeNode(treeNode -> treeNode.getModel().equals(event.stGroup))
				.forEach(treeNode -> treeModel.addNodeInSortedOrderAndSelect(treeNode, new STGroupActionTreeNode(event.action)));
	}
	...
	public void onSTGroupActionChanged(nextgen.events.STGroupActionChanged event) {
		findAllSTGroupActionTreeNode(treeNode -> treeNode.getModel().equals(event.sTGroupAction)).forEach(STGroupActionTreeNode::nodeChanged);
	}
	...
	@org.greenrobot.eventbus.Subscribe()
	public void onDomainVisitorChanged(nextgen.events.DomainVisitorChanged event) {
		findAllDomainVisitorTreeNode(treeNode -> treeNode.getModel().equals(event.domainVisitor)).forEach(DomainVisitorTreeNode::nodeChanged);
	}
	...
	public void onNewSTGroup(nextgen.events.NewSTGroup event) {
		findTemplatesTreeNode().ifPresent(treeNode -> treeModel.addNodeInSortedOrderAndSelect(treeNode, new STGroupTreeNode(event.group)));
	}

	@org.greenrobot.eventbus.Subscribe()
	public void onNewSTEnum(nextgen.events.NewSTEnum event) {
		findAllSTGroupTreeNode(treeNode -> treeNode.getModel().equals(event.stGroup))
							.forEach(treeNode -> treeModel.addNodeInSortedOrderAndSelect(treeNode, new STEnumTreeNode(event.stEnum)));
	}

	@org.greenrobot.eventbus.Subscribe()
	public void onNewSTInterface(nextgen.events.NewSTInterface event) {
		findAllSTGroupTreeNode(treeNode -> treeNode.getModel().equals(event.stGroup))
							.forEach(treeNode -> treeModel.addNodeInSortedOrderAndSelect(treeNode, new STInterfaceTreeNode(event.stInterface)));
	}
	...
	public void onNewSTGroupTemplate(nextgen.events.NewSTGroupTemplate event) {
		findAllSTGroupTreeNode(treeNode -> treeNode.getModel().equals(event.parent))
							.forEach(treeNode -> treeModel.addNodeInSortedOrderAndSelect(treeNode, new STTemplateTreeNode(event.template)));
	}
	...
	public void onNewSTGroupFile(nextgen.events.NewSTGroupFile event) {
		findAllSTGroupTreeNode(stGroupTreeNode -> stGroupTreeNode.getModel().equals(event.stGroupModel))
				.forEach(stGroupTreeNode -> treeModel.addNodeInSortedOrderAndSelect(stGroupTreeNode, new STGroupFileTreeNode(event.stGroupFile)));
	}
	...
	public class DomainTreeNode extends BaseTreeNode<nextgen.model.Domain> {
		...
		protected List<Action> getActions() {
			...
			appModel().doInTransaction(tx -> {
				actions.add(new AddDomainEntities(getModel(), tree));
				actions.add(new AddDomainVisitor(getModel(), tree));
			});
	...
	public class TemplatesTreeNode extends BaseTreeNode<String> {
		...
		TemplatesTreeNode(String model) {
			super(model, appModel().loadIcon("templates"));
	...
	public class DomainsTreeNode extends BaseTreeNode<String> {
		...
		DomainsTreeNode(String model) {
			super(model, appModel().loadIcon("domains"));
	...
	public class DomainEntityTreeNode extends BaseTreeNode<nextgen.model.DomainEntity> {
		...
		protected List<Action> getActions() {
			...
			appModel().doInTransaction(tx -> {
				actions.add(new DeleteDomainEntity(getModel(), tree));
				actions.add(new AddDomainRelation(getModel(), tree));
				actions.add(new EditDomainEntity(getModel(), tree));
			});
	...
	public class DomainRelationTreeNode extends BaseTreeNode<nextgen.model.DomainRelation> {
		...
		protected List<Action> getActions() {
			...
			appModel().doInTransaction(tx -> {
				actions.add(new SetDomainRelationType(getModel(), tree));
				actions.add(new DeleteDomainRelation(getModel(), tree));
			});
	...
	public class STGroupTreeNode extends BaseTreeNode<nextgen.model.STGroupModel> {
		...
		protected List<Action> getActions() {
			...
			appModel().doInTransaction(tx -> {
				actions.add(new NewAction(getModel(), tree));
				actions.add(new ImportSTTemplate(getModel(), tree));
				actions.add(new GenerateSTGroup(tree, getModel()));
				actions.add(new NewSTTemplate(getModel(), tree));
				actions.add(new NewEnum(getModel(), tree));
				actions.add(new NewInterface(getModel(), tree));
				actions.add(new RenameSTGroup(getModel(), tree));
				actions.add(new DeleteSTGroup(getModel(), tree));
				actions.add(new AddFileSinkToGroup(getModel()));
				actions.add(new SetSTGroupLanguage(tree, getModel()));
			});
	...
	public class STEnumTreeNode extends BaseTreeNode<nextgen.model.STEnum> {
		...
		protected List<Action> getActions() {
			...
			appModel().doInTransaction(tx -> {
				getParentNode(STGroupTreeNode.class).ifPresent(parent -> actions.add(new RenameEnum(getModel(), parent.getModel(), tree)));
				getParentNode(STGroupTreeNode.class).ifPresent(parent -> actions.add(new DeleteEnum(getModel(), parent.getModel(), tree)));
				actions.add(new EditEnum(getModel(), tree));
			});
		...
		protected List<Action> getActions() {
			...
			appModel().doInTransaction(tx -> {
				...
				getParentNode(STGroupTreeNode.class).ifPresent(parent -> { 
					...
					if (!candidateChildren.isEmpty()) actions.add(new AddChildrenToTemplate("Add " + candidateChildren.size() + " templates as children", parent.getModel(), getModel(), candidateChildren, tree));
					...
					if (!childTemplates.isEmpty()) actions.add(new AddInterface("Add interfaces to children", childTemplates, tree));
					actions.add(new SetInterfaces(parent.getModel(), getModel(), tree));
					getModel().getImplements().forEach(implement -> actions.add(new RemoveInterfaceFromSTTemplate("Remove " + implement, parent.getModel(), getModel(), implement, tree)));
					actions.add(new RenameSTTemplate(getModel(), parent.getModel(), tree));
				});
				actions.add(new DeleteSTTemplate(getModel(), tree));
	...
	public class STInterfaceTreeNode extends BaseTreeNode<nextgen.model.STInterface> {
		...
		protected List<Action> getActions() {
			...
			appModel().doInTransaction(tx -> {
				getParentNode(STGroupTreeNode.class).ifPresent(parent -> actions.add(new RenameSTInterface(getModel(), parent.getModel(), tree)));
				getParentNode(STGroupTreeNode.class).ifPresent(parent -> actions.add(new DeleteSTInterface(getModel(), parent.getModel(), tree)));
			});
	...
	public class STGroupFileTreeNode extends BaseTreeNode<nextgen.model.STGroupFile> {
		...
		protected List<Action> getActions() {
			...
			appModel().doInTransaction(tx -> {
				actions.add(new GenerateSTGroupFromFile(getModel(), tree));
				actions.add(new DeleteSTGroupFile(getModel(), tree));
			});
