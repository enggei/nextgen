...
public class City {
	...
	public City addAddresses(Address dst) { 
		...
		node.createRelationshipTo(dst.getNode(), org.neo4j.graphdb.RelationshipType.withName("addresses"));
		return this;
	}

	public java.util.stream.Stream<Address> getAddresses() { 
		return java.util.stream.StreamSupport.stream(node.getRelationships(org.neo4j.graphdb.Direction.OUTGOING, org.neo4j.graphdb.RelationshipType.withName("addresses")).spliterator(), false).map((relationship) -> new Address(relationship.getOtherNode(node)));
	}

	public City removeAddresses(Address dst) { 
		final java.util.Optional<org.neo4j.graphdb.Relationship> existing = java.util.stream.StreamSupport.stream(node.getRelationships(org.neo4j.graphdb.Direction.OUTGOING, org.neo4j.graphdb.RelationshipType.withName("addresses")).spliterator(), false).filter((r) -> r.getOtherNode(node).equals(dst.getNode())).findAny();
