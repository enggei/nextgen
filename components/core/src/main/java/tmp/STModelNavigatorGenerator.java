...
public class STModelNavigatorGenerator {
	...
	public static TreeNavigator generate(){ 
		return NextgenST.newTreeNavigator().setPackageName("nextgen.st").setName("STModelNavigator").addFields("STWorkspace", "workspace").setRootNodeExpression("new RootNode(\"Projects\")").setPreferredWidth("800").setPreferredHeight("600").addConstructorStatements("org.greenrobot.eventbus.EventBus.getDefault().register(this);").addTreeNodesSelected("RootNode").addTreeNodesSelected("STProjectTreeNode").addTreeNodesSelected("ModelsTreeNode").addTreeNodesSelected("STGroupModelTreeNode").addTreeNodesSelected("STTemplateTreeNode").addTreeNodesSelected("STModelTreeNode").addTreeNodesSelected("STFileSinkTreeNode").addTreeNodesSelected("STParameterTreeNode").addTreeNodesSelected("STModelArgumentTreeNode").addTreeNodesSelected("STValueTreeNode").addTreeNodesSelected("STValueArgumentTreeNode").addTreeNodesSelected("STKVArgumentTreeNode").addTreeNodesSelected("STKVTreeNode").addTreeNodesSelected("STModelKVArgumentTreeNode").addTreeNodesSelected("STValueKVArgumentTreeNode").addEvents(NextgenST.newEventSubscription().setEventName("NewSTProject").addStatements("findRootNode().ifPresent(treeNode -> treeModel.addNodeInSortedOrder(treeNode, new STProjectTreeNode(event.project)));")).addEvents(NextgenST.newEventSubscription().setEventName("NewFileSink").addStatements("findSTModelTreeNode(treeNode -> treeNode.getModel().equals(event.stModel)).ifPresent(treeNode -> treeModel.addNodeInSortedOrder(treeNode, new STFileSinkTreeNode(event.stFile)));")).addEvents(NextgenST.newEventSubscription().setEventName("NewSTArgument").addStatements("treeModel.find(treeNode -> treeNode.getModel().equals(event.model))\n" + 
					...
					"				.onSingleSTValue((stArgument, stValue) -> treeModel.addNodeInSortedOrderAndSelect(stParameterTreeNode, new nextgen.swing.STModelNavigator.STValueArgumentTreeNode(stValue, stArgument)))\n" +
					"				.onSingleSTModel((stArgument, stValue) -> treeModel.addNodeInSortedOrderAndSelect(stParameterTreeNode, new nextgen.swing.STModelNavigator.STModelArgumentTreeNode(stValue.getStModel(), stArgument)))\n" +
					"				.onListSTValue((stArgument, stValue) -> treeModel.addNodeInSortedOrderAndSelect(stParameterTreeNode, new nextgen.swing.STModelNavigator.STValueArgumentTreeNode(stValue, stArgument)))\n" +
					"				.onListSTModel((stArgument, stValue) -> treeModel.addNodeInSortedOrderAndSelect(stParameterTreeNode, new nextgen.swing.STModelNavigator.STModelArgumentTreeNode(stValue.getStModel(), stArgument)))\n" +
					"				.onKVListConsumer((stArgument, stKVValues) -> treeModel.addNodeInSortedOrderAndSelect(stParameterTreeNode, new nextgen.swing.STModelNavigator.STKVArgumentTreeNode(stArgument, event.parameter)))\n" +
					...
					"		.ifPresent(treeNode -> treeModel.addNodeInSortedOrder(treeNode, new nextgen.swing.STModelNavigator.STKVArgumentTreeNode(event.argument, event.stParameter)));")).addEvents(NextgenST.newEventSubscription().setEventName("NewKV").addStatements("findSTKVArgumentTreeNode(treeNode -> treeNode.uuid.equals(event.argument.getUuid()))\n" +
					"		.ifPresent(treeNode -> {\n" + 
					"			treeModel.addNodeInSortedOrderAndSelect(treeNode, new nextgen.swing.STModelNavigator.STKVTreeNode(event.kv, treeNode.getModel(), event.stParameterKey));\n" +
					...
					"		.ifPresent(stTemplateTreeNode -> treeModel.addNodeInSortedOrderAndSelect(stTemplateTreeNode, new nextgen.swing.STModelNavigator.STModelTreeNode(event.model, event.template)));")).addEvents(NextgenST.newEventSubscription().setEventName("NewSTProjectSTModel").addStatements("findSTProjectTreeNode(stProjectTreeNode -> stProjectTreeNode.getModel().equals(event.project))\n" +
					"		.ifPresent(stProjectTreeNode -> {\n" + 
					"			final nextgen.st.model.STGroupModel stGroup = appModel().findSTGroup(event.template);\n" +
					"			final java.util.Optional<nextgen.swing.STModelNavigator.STGroupModelTreeNode> groupModelTreeNode = findSTGroupModelTreeNode(stProjectTreeNode, stGroupModelTreeNode -> stGroupModelTreeNode.getModel().equals(stGroup));\n" +
					"			if (groupModelTreeNode.isPresent()) {\n" + 
					"				final java.util.Optional<nextgen.swing.STModelNavigator.STTemplateTreeNode> stTemplateTreeNode = findSTTemplateTreeNode(groupModelTreeNode.get(), stTemplateTreeNode1 -> stTemplateTreeNode1.getModel().equals(event.template));\n" +
					"				if (stTemplateTreeNode.isPresent()) {\n" + 
					"					treeModel.addNodeInSortedOrderAndSelect(stTemplateTreeNode.get(), new nextgen.swing.STModelNavigator.STModelTreeNode(event.model, event.template));\n" +
					"				} else {\n" + 
					"					final nextgen.swing.STModelNavigator.STTemplateTreeNode newSTTemplateTreeNode = new nextgen.swing.STModelNavigator.STTemplateTreeNode(event.template);\n" +
					"					treeModel.addNodeInSortedOrder(groupModelTreeNode.get(), newSTTemplateTreeNode);\n" + 
					"					treeModel.addNodeInSortedOrderAndSelect(newSTTemplateTreeNode, new nextgen.swing.STModelNavigator.STModelTreeNode(event.model, event.template));\n" +
					"				}\n" + 
					"\n" + 
					"			} else {\n" + 
					"				final nextgen.swing.STModelNavigator.STGroupModelTreeNode stGroupModelTreeNode = new nextgen.swing.STModelNavigator.STGroupModelTreeNode(stGroup);\n" +
					"				treeModel.addNodeInSortedOrder(stProjectTreeNode, stGroupModelTreeNode);\n" + 
					"				final nextgen.swing.STModelNavigator.STTemplateTreeNode stTemplateTreeNode = new nextgen.swing.STModelNavigator.STTemplateTreeNode(event.template);\n" +
					"				treeModel.addNodeInSortedOrder(stGroupModelTreeNode, stTemplateTreeNode);\n" + 
					"				treeModel.addNodeInSortedOrderAndSelect(stTemplateTreeNode, new nextgen.swing.STModelNavigator.STModelTreeNode(event.model, event.template));\n" +
					"			}\n" + 
					...
					"		final STModelNavigator.STTemplateTreeNode stTemplateTreeNode = new nextgen.swing.STModelNavigator.STTemplateTreeNode(stTemplate);\n" +
					"		stGroupTreeNodeMap.get(stGroup).add(stTemplateTreeNode);\n" + 
					"		stTemplateTreeNodeMap.put(stTemplate, stTemplateTreeNode);\n" + 
					"	}\n" + 
					"\n" + 
					"	stTemplateTreeNodeMap.get(stTemplate).add(new nextgen.swing.STModelNavigator.STModelTreeNode(stModel, stTemplate));\n" +
					"});\n" + 
					"\n" + 
					"model.getValuesSorted().forEach(stValue -> add(new nextgen.swing.STModelNavigator.STValueTreeNode(stValue)));").addActions("new nextgen.actions.AddValueToProjectFromInput(getModel(), tree)").addActions("new nextgen.actions.AddMultipleValuesToProject(getModel(), tree)").addActions("new nextgen.actions.GenerateAllProjectModels(getModel())").addSelectionStatements("nextgen.events.ModelNavigatorSTProjectTreeNodeClicked.post(selectedNode.getModel());")).addTreeNodes(NextgenST.newTreeNode().setName("ModelsTreeNode").setModelType("String").setIcon("appModel().loadIcon(\"sq-teal\")").setLabelExpression("getModel()").addConstructorStatements("appModel().doInTransaction(transaction -> {\n" +
					...
					"	final nextgen.swing.STModelNavigator.STTemplateTreeNode stTemplateTreeNode = new nextgen.swing.STModelNavigator.STTemplateTreeNode(stTemplate);\n" +
					...
					"			.onKVListConsumer((stArgument, stKVValues) -> addChild(new STKVArgumentTreeNode(stArgument, model))));").addGetActionsStatements("final java.util.Optional<nextgen.swing.STModelNavigator.STModelTreeNode> parentNode = getParentNode(nextgen.swing.STModelNavigator.STModelTreeNode.class);").addGetActionsStatements("final java.util.List<nextgen.st.model.STModel> selectedSTModels = getSelectedSTModels()\n" +
