...
public class STModelNode extends STNode {
    ...
    public Stream<UUID> getOutgoingReferences() {
        return stModel.getArguments().map(STArgument::getValue).map(stValue -> (stValue.getType().equals(STValueType.STMODEL)) ? UUID.fromString(stValue.getStModel().getUuid()) : UUID.fromString(stValue.getUuid()));
    }
