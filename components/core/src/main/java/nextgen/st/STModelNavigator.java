...
public class STModelNavigator extends JPanel {
   ...
   public class STProjectTreeNode extends BaseTreeNode<nextgen.st.model.STProject> {
      ...
      protected List<Action> getActions() {
         ...
         getSelectedSTModelTreeNodes().forEach(stModelTreeNode -> actions
               .add(new nextgen.actions.AddModelToProject("Add " + stModelTreeNode.getLabel(), getModel(), stModelTreeNode.getModel())));
   ...
   public class ModelsTreeNode extends BaseTreeNode<String> {
      ...
      private void addSTTemplateChild(nextgen.st.domain.STTemplate stTemplate, BaseTreeNode<?> parent) {
         ...
         appModel().db.findAllSTModelByStTemplate(stTemplate.getUuid())
                      .forEach(stModel -> stTemplateTreeNode.add(new STModelTreeNode(stModel, stTemplate, null)));
      ...
      STParameterTreeNode(nextgen.st.domain.STParameter model, nextgen.st.model.STModel stModel) {
         ...
         stModel.getArgumentsSorted()
                ...
                .forEach(appModel().stArgumentConsumer(model)
                                   ...
                                   .onSingleSTModel((stArgument, stValue) -> add(new STModelTreeNode(stValue.getStModel(), appModel()
                                         .findSTTemplateByUuid(stValue.getStModel().getStTemplate()), stArgument)))
                                   .onListSTValue((stArgument, stValue) -> add(new STValueTreeNode(stValue, stArgument)))
                                   .onListSTModel((stArgument, stValue) -> add(new STModelTreeNode(stValue.getStModel(), appModel()
                                         .findSTTemplateByUuid(stValue.getStModel().getStTemplate()), stArgument)))
   ...
   public void onSTArgumentAdded(nextgen.events.NewSTArgument event) {
      findSTModelTreeNode(treeNode -> treeNode.getModel().equals(event.stModel))
            .ifPresent(stModelTreeNode -> findSTParameterTreeNode(stModelTreeNode, stParameterTreeNode -> stParameterTreeNode.getModel().getUuid()
                                                                                                                             .equals(event.stArgument
                                                                                                                                   .getStParameter()))
                  .ifPresent(stParameterTreeNode -> {
                     appModel().stArgumentConsumer(event.stParameter)
                               .onSingleSTValue((stArgument, stValue) -> treeModel
                                     .addNodeInSortedOrder(stParameterTreeNode, new STValueTreeNode(stValue, event.stArgument)))
                               .onSingleSTModel((stArgument, stValue) -> treeModel
                                     .addNodeInSortedOrder(stParameterTreeNode, new STModelTreeNode(stValue.getStModel(), appModel()
                                           .findSTTemplateByUuid(stValue.getStModel().getStTemplate()), event.stArgument)))
                               .onListSTValue((stArgument, stValue) -> treeModel
                                     .addNodeInSortedOrder(stParameterTreeNode, new STValueTreeNode(stValue, event.stArgument)))
                               .onListSTModel((stArgument, stValue) -> treeModel
                                     .addNodeInSortedOrder(stParameterTreeNode, new STModelTreeNode(stValue.getStModel(), appModel()
                                           .findSTTemplateByUuid(stValue.getStModel().getStTemplate()), event.stArgument)))
                               .onKVListConsumer((stArgument, stKVValues) -> treeModel
                                     .addNodeInSortedOrder(stParameterTreeNode, new STKVArgumentTreeNode(event.stArgument, event.stParameter)));
                     stModelTreeNode.nodeChanged();
                  }));
   }

   @org.greenrobot.eventbus.Subscribe()
   public void onSTProjectSTModel(nextgen.events.NewSTProjectSTModel event) {
      findSTProjectTreeNode(stProjectTreeNode -> stProjectTreeNode.getModel().equals(event.stProject))
   ...
   public void onSTModelDeleted(nextgen.events.STModelDeleted event) {
      findModelsTreeNode(modelsTreeNode -> true)
   ...
   public void onSTModelCanvasNodeClicked(nextgen.events.CanvasSTModelClicked event) {
      SwingUtilities.invokeLater(() -> {
         ...
         final TreePath path = rootNode
               .find(baseTreeNode -> isSTModelTreeNode(baseTreeNode) && ((STModelTreeNode) baseTreeNode).getModel().equals(event.model));
         if (path != null) {
            tree.scrollPathToVisible(path);
            tree.setSelectionPath(path);
         }
