...
public class STModelNavigator extends JPanel {
   ...
   public void onNewSTProject(nextgen.events.NewSTProject event) {
      System.out.println("NewSTProject");
   ...
   public void onNewFileSink(nextgen.events.NewFileSink event) {
      ...
      findSTModelTreeNode(treeNode -> treeNode.getModel()
            .equals(event.stModel)).ifPresent(treeNode -> treeModel.addNodeInSortedOrder(treeNode, new STFileSinkTreeNode(event.stFile)));
   }

   @org.greenrobot.eventbus.Subscribe()
   public void onNewSTArgument(nextgen.events.NewSTArgument event) {
      System.out.println("NewSTArgument");
      treeModel.find(treeNode -> treeNode.getModel()
            .equals(event.model))
            .flatMap(treeNode -> findSTParameterTreeNode(treeNode, stParameterTreeNode -> stParameterTreeNode.getModel()
                  .equals(event.parameter)))
   ...
   public void onNewSTModel(nextgen.events.NewSTModel event) {
      ...
      findModelsTreeNode()
            .flatMap(modelsTreeNode -> findSTGroupModelTreeNode(modelsTreeNode, treeNode -> treeNode.getModel()
                  .equals(event.stGroup))
                  .flatMap(treeNode -> findSTTemplateTreeNode(treeNode, stTemplateTreeNode -> stTemplateTreeNode.getModel()
                        .equals(event.template))))
   ...
   public void onNewSTProjectSTModel(nextgen.events.NewSTProjectSTModel event) {
      ...
      findSTProjectTreeNode(stProjectTreeNode -> stProjectTreeNode.getModel()
            .equals(event.project))
            .ifPresent(stProjectTreeNode -> {
               final nextgen.st.domain.STGroupModel stGroup = appModel().findSTGroup(event.template);
               final java.util.Optional<nextgen.st.STModelNavigator.STGroupModelTreeNode> groupModelTreeNode = findSTGroupModelTreeNode(stProjectTreeNode, stGroupModelTreeNode -> stGroupModelTreeNode.getModel()
                     .equals(stGroup));
               if (groupModelTreeNode.isPresent()) {
                  final java.util.Optional<nextgen.st.STModelNavigator.STTemplateTreeNode> stTemplateTreeNode = findSTTemplateTreeNode(groupModelTreeNode.get(), stTemplateTreeNode1 -> stTemplateTreeNode1.getModel()
                        .equals(event.template));
   ...
   public void onNewSTProjectSTValue(nextgen.events.NewSTProjectSTValue event) {
      System.out.println("NewSTProjectSTValue");
      findSTProjectTreeNode(treeNode -> treeNode.getModel()
            .equals(event.project))
   ...
   public void onSTValueChanged(nextgen.events.STValueChanged event) {
      System.out.println("STValueChanged");
      treeModel.find(treeNode -> treeNode.getModel()
            .equals(event.value))
            .ifPresent(nextgen.st.STModelNavigator.BaseTreeNode::nodeChanged);
   }

   @org.greenrobot.eventbus.Subscribe()
   public void onSTArgumentChanged(nextgen.events.STArgumentChanged event) {
      System.out.println("STArgumentChanged");
         ...
         model.getModelsSorted()
               .forEach(stModel -> {
                  ...
                  if (!stTemplateTreeNodeMap.containsKey(stTemplate)) {
                     ...
                     stGroupTreeNodeMap.get(stGroup)
                           .add(stTemplateTreeNode);
                     stTemplateTreeNodeMap.put(stTemplate, stTemplateTreeNode);
                  }

                  stTemplateTreeNodeMap.get(stTemplate)
                        .add(new nextgen.st.STModelNavigator.STModelTreeNode(stModel, stTemplate));
               });

         model.getValuesSorted()
               .forEach(stValue -> add(new nextgen.st.STModelNavigator.STValueTreeNode(stValue)));
      }
   ...
   public class ModelsTreeNode extends BaseTreeNode<String> {
            ...
            appModel().getGroupModels()
                  .forEach(stGroupModel -> {
                     ...
                     stGroupModel.getTemplates()
                           .forEach(stTemplate -> addSTTemplateChild(stTemplate, stGroupModelTreeNode));
                  });
         });
      }
      ...
      private void addSTTemplateChild(nextgen.st.domain.STTemplate stTemplate, BaseTreeNode<?> parent) {
         ...
         appModel().db.findAllSTModelByStTemplate(stTemplate.getUuid())
               .forEach(stModel -> stTemplateTreeNode.add(new STModelTreeNode(stModel, stTemplate)));
         stTemplate.getChildren()
               .forEach(stTemplateChild -> addSTTemplateChild(stTemplateChild, stTemplateTreeNode));
      }
   }
   ...
   public class STGroupModelTreeNode extends BaseTreeNode<nextgen.st.domain.STGroupModel> {
      ...
      private boolean countChildren(nextgen.st.domain.STTemplate stTemplate) {
         long count = appModel().findAllSTModelByStTemplate(stTemplate.getUuid())
               .count();
         if (count != 0L) return true;
         Iterator<nextgen.st.domain.STTemplate> iterator = stTemplate.getChildren()
               .iterator();
      ...
      protected List<Action> getActions() {
         ...
         appModel().doInTransaction(tx -> {
            ...
            final java.util.List<nextgen.st.model.STModel> selectedSTModels = getSelectedSTModels()
                  ...
                  .filter(stModel1 -> !stModel1.equals(parentNode.get()
                        .getModel()))
                  .collect(java.util.stream.Collectors.toList());
   ...
   public <T> java.util.stream.Stream<T> getSelectedNodes(Class<T> type) {
      ...
      return Arrays.stream(selectionPaths)
            ...
            .filter(treePath -> treePath.getLastPathComponent()
                  .getClass()
                  .isAssignableFrom(type))
   ...
   class STModelNavigatorTreeModel extends DefaultTreeModel {
      ...
      protected <T extends BaseTreeNode<?>> Optional<T> find(Class<T> nodeType) {
         ...
         if (root.getClass()
               .isAssignableFrom(nodeType)) return Optional.of((T) root);
         return find((BaseTreeNode<?>) getRoot(), navigatorTreeNode ->
               navigatorTreeNode.getClass()
      ...
      protected <T extends BaseTreeNode<?>> Optional<T> find(Class<T> nodeType, java.util.function.Predicate<T> predicate) {
         final BaseTreeNode<?> root = (BaseTreeNode<?>) getRoot();
         if (root.getClass()
               .isAssignableFrom(nodeType) && predicate.test((T) root)) return Optional.of((T) root);
