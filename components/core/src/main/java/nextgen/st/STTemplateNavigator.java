...
public class STTemplateNavigator extends JPanel {
         ...
         public void mouseClicked(MouseEvent e) {
            if (SwingUtilities.isRightMouseButton(e)) {
               ...
               if (lastPathComponent instanceof STGroupTreeNode) {
                  ...
                  selectedNode.getParentNode(STGroupTreeNode.class)
                        .ifPresent(stGroupTreeNode -> workspace.getSTEditor(stGroupTreeNode.getModel())
      ...
      protected List<Action> getActions() {
         ...
         actions.add(newAction("New STGroup", actionEvent -> {
            SwingUtil.getInputFromUser(tree, "Name").ifPresent(name -> {
               ...
               final Optional<nextgen.st.domain.STGroupModel> existing = getModel().getGroups()
         ...
         actions.add(newAction("Generate All", actionEvent -> {
            SwingUtilities.invokeLater(() -> getChildren(STGroupTreeNode.class).forEach(stGroupTreeNode -> presentationModel
                  .generateSTGroup(stGroupTreeNode.getModel(), false)));
         }));
         actions.add(newAction("Generate Group and Neo models", actionEvent -> {
            SwingUtilities.invokeLater(() -> getChildren(STGroupTreeNode.class).forEach(stGroupTreeNode -> presentationModel
                  .generateSTGroup(stGroupTreeNode.getModel(), true)));
         }));
         actions.add(newAction("Expand", actionEvent -> {
            SwingUtilities.invokeLater(() -> expandTreeNodesRecursive(getThisPath(), true));
         }));
   ...
   public class STEnumTreeNode extends BaseTreeNode<nextgen.st.domain.STEnum> {
      ...
      protected List<Action> getActions() {
         ...
         actions.add(newAction("Edit", actionEvent -> {
            ...
            final JPanel contentPanel = new JPanel(new GridLayout((int) getModel().getValues()
                  .count() + newFields + 1, 2));
   ...
   public class STTemplateTreeNode extends BaseTreeNode<nextgen.st.domain.STTemplate> {
      ...
      protected List<Action> getActions() {
         final List<Action> actions = super.getActions();
         final Set<STTemplateTreeNode> candidateChildren = getSelectedNodes(STTemplateTreeNode.class).filter(stTemplateTreeNode -> !stTemplateTreeNode
               .equals(this)).collect(java.util.stream.Collectors.toSet());
         ...
         final List<nextgen.st.domain.STTemplate> childTemplates = getModel().getChildren()
               ...
               getModel().getParameters().forEach(stParameter -> {
                  ...
                  switch (stParameter.getType()) {
                     ...
                     case LIST:
                        ...
                        stParameter.setArgumentType(types.length() == 0 ? (stParameter.getName()
                              .startsWith("is") ? "Boolean" : "Object") : types);
                        break;

                     case KVLIST:
                        stParameter.getKeys().forEach(stParameterKey -> {
                           final JTextField txtKVTypes = txtParameterMap.get(stParameter.getName() + "." + stParameterKey
                                 .getName());
                           final String kvTypes = txtKVTypes.getText().trim();
                           stParameterKey.setArgumentType(kvTypes.length() == 0 ? (stParameterKey.getName()
            ...
            SwingUtil.confirm(tree, "Delete " + getModel().getName())
                  .ifPresent(aBoolean -> {
                     ...
                     SwingUtilities.invokeLater(() -> {
                        ...
                        getParentNode(STGroupTreeNode.class).ifPresent(stGroupTreeNode -> workspace.getSTEditor(stGroupTreeNode
                              .getModel()).setSTTemplate(null));
                     });
                  });
         }));
         return actions;
