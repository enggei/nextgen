...
public class STTemplateNavigator extends JPanel {
            ...
            if (SwingUtilities.isRightMouseButton(e)) {
               ...
               if (lastPathComponent instanceof STGroupTreeNode) {
                  ...
                  selectedNode
                        ...
                        .ifPresent(stGroupTreeNode -> workspace
                              .getSTEditor(stGroupTreeNode.getModel())
                              .setSTTemplate(selectedNode.getModel()));
                  ...
                  selectedNode
                        ...
                        .ifPresent(stGroupTreeNode -> workspace
                              .getSTEditor(stGroupTreeNode.getModel())
                              .setSTEnum(selectedNode.getModel()));
                  ...
                  selectedNode
                        ...
                        .ifPresent(stGroupTreeNode -> workspace
                              .getSTEditor(stGroupTreeNode.getModel())
                              .setSTInterface(selectedNode.getModel()));
               }
            }
         }
      });
   ...
   public class RootNode extends BaseTreeNode<String> {
      ...
      protected List<Action> getActions() {
         ...
         actions.add(newAction("New STGroup", actionEvent -> {
            SwingUtil
                  .getInputFromUser(tree, "Name")
                  .ifPresent(name -> {

                     final Optional<nextgen.st.domain.STGroupModel> existing = appModel().stGroups
                           .stream()
                           .filter(groupModel -> groupModel
                                 .getName()
                                 .toLowerCase()
                                 .equals(name))
                           .findAny();
   ...
   public class STGroupTreeNode extends BaseTreeNode<nextgen.st.domain.STGroupModel> {
      ...
      STGroupTreeNode(nextgen.st.domain.STGroupModel model) {
         ...
         model
               ...
               .forEach(stEnum -> add(new STEnumTreeNode(stEnum)));
         model
               .getTemplates()
               .sorted((o1, o2) -> o1
                     .getName()
                     .compareToIgnoreCase(o2.getName()))
               .forEach(stTemplate -> add(new STTemplateTreeNode(stTemplate)));
         model
               .getInterfaces()
               .sorted((o1, o2) -> o1
                     .getName()
                     .compareToIgnoreCase(o2.getName()))
               .forEach(stInterface -> add(new STInterfaceTreeNode(stInterface)));
      }
      ...
      protected List<Action> getActions() {
         ...
         actions.add(newAction("Import From File", actionEvent -> {
            SwingUtil
                  .showOpenFile(tree, appModel().getLastDir())
                  .ifPresent(file -> {
         ...
         actions.add(newAction("New template", actionEvent -> {
            SwingUtil
                  .getInputFromUser(tree, "Name")
                  .ifPresent(name ->
                        STAppPresentationModel
                              .isValidTemplateName(tree, getModel(), name)
                              .ifPresent(s -> {
         ...
         actions.add(newAction("Delete", actionEvent -> {
            SwingUtil
                  .confirm(tree, "Delete " + getModel().getName())
         ...
         actions.add(newAction("Set Icon name", actionEvent -> {
            SwingUtil
                  .getInputFromUser(tree, "Icon name")
                  .ifPresent(iconName -> {
   ...
   public class STEnumTreeNode extends BaseTreeNode<nextgen.st.domain.STEnum> {
      ...
      protected List<Action> getActions() {
         ...
         actions.add(newAction("Edit", actionEvent -> {
            ...
            final JPanel contentPanel = new JPanel(new GridLayout((int) getModel()
                  .getValues()
                  .count() + newFields + 1, 2));
            ...
            final JButton btnSave = new JButton(newAction("Save", actionEvent1 -> {
               ...
               for (STEnumValue stEnumValue : txtEnumValuesName.keySet()) {
                  final String txtEnumValueName = txtEnumValuesName
                        .get(stEnumValue)
                        .getText()
                        .trim();
                  final String txtEnumValueLexical = txtEnumLexical
                        .get(stEnumValue)
                        .getText()
                        .trim();
               ...
               for (int i = 0; i < newFields; i++) {
                  final String newEnumValue = newTxtEnumValuesName
                        .get(i)
                        .getText()
                        .trim();
                  final String newEnumLexical = newTxtEnumLexical
                        .get(i)
                        .getText()
                        .trim();
                  if (newEnumValue.length() == 0) continue;

                  getModel().addValues(STJsonFactory
                        .newSTEnumValue()
         ...
         actions.add(newAction("Rename", actionEvent -> {
            SwingUtil
                  ...
                  .ifPresent(candidateName -> getParentNode(STGroupTreeNode.class)
                        .ifPresent(stGroupTreeNode -> STAppPresentationModel
                              .isValidTemplateName(tree, stGroupTreeNode.getModel(), candidateName)
                              .ifPresent(name -> {
                                 getModel().setName(name);
                                 stGroupTreeNode.save();
                                 SwingUtilities.invokeLater(() -> {
   ...
   public class STTemplateTreeNode extends BaseTreeNode<nextgen.st.domain.STTemplate> {
      ...
      protected List<Action> getActions() {
         final List<Action> actions = super.getActions();
         final Set<STTemplateTreeNode> candidateChildren = getSelectedNodes(STTemplateTreeNode.class)
               ...
               SwingUtilities.invokeLater(() -> {
                  for (STTemplateTreeNode stTemplateTreeNode : candidateChildren) {
                     ...
                     if (parent instanceof STGroupTreeNode)
                        ((STGroupTreeNode) parent)
                              .getModel()
                              .removeTemplates(stTemplateTreeNode.getModel());
                     else if (parent instanceof STTemplateTreeNode)
                        ((STTemplateTreeNode) parent)
                              .getModel()
                              .removeChildren(stTemplateTreeNode.getModel());
         ...
         final List<nextgen.st.domain.STTemplate> childTemplates = getModel()
               .getChildren()
         ...
         if (!childTemplates.isEmpty())
            actions.add(newAction("Set interfaces on " + childTemplates.size() + " children", actionEvent -> {
               ...
               final JButton btnSave = new JButton(newAction("Save", actionEvent1 -> {
                  ...
                  final String interfaceName = txtImplements
                        .getText()
                        .trim();

                  childTemplates.forEach(stTemplate -> stTemplate
                        .getImplements()
         ...
         getModel()
               .getImplements()
               .forEach(implement -> actions.add(newAction("Remove interface '" + implement + "'", actionEvent -> {
         ...
         actions.add(newAction("New Instance", actionEvent -> {
            getParentNode(STGroupTreeNode.class)
                  .ifPresent(stGroupTreeNode -> appModel().doLaterInTransaction(transaction -> {
                     appModel().db.newSTModel(stGroupTreeNode
                           .getModel()
                           .getUuid(), getModel());
                     workspace.setSelectedComponent(workspace.findCanvas());
                  }));
                     ...
                     .forEach(stParameter -> {
                        ...
                        switch (stParameter.getType()) {
                           ...
                           case LIST:
                              ...
                              final String types = txtTypes
                                    .getText()
                                    .trim();
                              stParameter.setArgumentType(types.length() == 0 ? (stParameter
                                    .getName()
                                    .startsWith("is") ? "Boolean" : "Object") : types);
                              break;
                           ...
                           case KVLIST:
                              stParameter
                                    ...
                                    .forEach(stParameterKey -> {
                                       ...
                                       final String kvTypes = txtKVTypes
                                             .getText()
                                             .trim();
                                       stParameterKey.setArgumentType(kvTypes.length() == 0 ? (stParameterKey
                                             .getName()
                                             .startsWith("is") ? "Boolean" : "Object") : kvTypes);
                                    });
                              break;
                        }
                     });
         ...
         actions.add(newAction("Set interfaces", actionEvent -> {
            ...
            final JButton btnSave = new JButton(newAction("Save", actionEvent1 -> {
               ...
               for (JTextField txtImplement : txtImplements) {
                  final String trim = txtImplement
                        .getText()
                        .trim();
            ...
            SwingUtil
                  ...
                  .ifPresent(aBoolean -> {
                     ...
                     SwingUtilities.invokeLater(() -> {
                        ...
                        getParentNode(STGroupTreeNode.class).ifPresent(stGroupTreeNode -> workspace
                              .getSTEditor(stGroupTreeNode.getModel())
                              .setSTTemplate(null));
                     });
                  });
   ...
   public void onNewSTTemplate(nextgen.events.NewSTTemplate event) {
      ...
      final Object parent = event.parent;
      if (parent instanceof STGroupModel)
         treeModel
               .find(STGroupTreeNode.class, stGroupTreeNode -> stGroupTreeNode
                     .getModel()
                     .equals(parent))
               .ifPresent(stGroupTreeNode -> treeModel.addNodeInSortedOrder(stGroupTreeNode, new nextgen.st.STTemplateNavigator.STTemplateTreeNode(event.model)));
      else if (parent instanceof nextgen.st.domain.STTemplate)
         treeModel
               .find(STTemplateTreeNode.class, stTemplateTreeNode ->
                     stTemplateTreeNode
                           .getModel()
                           .equals(parent))
               .ifPresent(stTemplateTreeNode -> treeModel.addNodeInSortedOrder(stTemplateTreeNode, new nextgen.st.STTemplateNavigator.STTemplateTreeNode(event.model)));
   ...
   public void onSTModelEditorTreeNodeClicked(nextgen.st.STAppEvents.STModelEditorTreeNodeClicked event) {
      ...
      final TreePath path = rootNode.find(baseTreeNode -> (baseTreeNode instanceof STTemplateTreeNode) && ((STTemplateTreeNode) baseTreeNode)
            .getModel()
            .equals(stTemplate));
      if (path != null) {
         tree.scrollPathToVisible(path);
   ...
   class STTemplateNavigatorTreeModel extends DefaultTreeModel {
      ...
      protected <T extends BaseTreeNode<?>> Optional<T> find(Class<T> nodeType, java.util.function.Predicate<T> predicate) {
         return find((BaseTreeNode<?>) getRoot(), navigatorTreeNode -> navigatorTreeNode
               .getClass()
      ...
      protected <T extends BaseTreeNode<?>> Optional<T> find(BaseTreeNode<?> parent, Class<T> nodeType, java.util.function.Predicate<BaseTreeNode<?>> predicate) {
         return find(parent, navigatorTreeNode -> navigatorTreeNode
               .getClass()
               .isAssignableFrom(nodeType) && predicate.test((T) navigatorTreeNode));
      }
