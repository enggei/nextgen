...
public class STTemplateNavigator extends JPanel {
   ...
   public class STGroupTreeNode extends BaseTreeNode<nextgen.st.domain.STGroupModel> {
      ...
      STGroupTreeNode(nextgen.st.domain.STGroupModel model) {
         ...
         model.getInterfaces().sorted((o1, o2) -> o1.getName().compareToIgnoreCase(o2.getName()))
   ...
   public class STEnumTreeNode extends BaseTreeNode<nextgen.st.domain.STEnum> {
      ...
      protected List<Action> getActions() {
         ...
         getParentNode(STGroupTreeNode.class)
               .ifPresent(stGroupTreeNode -> actions.add(new nextgen.actions.RenameEnum(getModel(), stGroupTreeNode.getModel(), tree)));
         getParentNode(STGroupTreeNode.class)
               .ifPresent(stGroupTreeNode -> actions.add(new nextgen.actions.DeleteEnum(getModel(), stGroupTreeNode.getModel(), tree)));
         return actions;
      }
   ...
   private void onSTEnumTreeNodeSelected(STEnumTreeNode selectedNode) {
      selectedNode.getParentNode(STGroupTreeNode.class)
                  .ifPresent(stGroupTreeNode -> workspace.getSTEditor(stGroupTreeNode.getModel()).setSTEnum(selectedNode.getModel()));
   }
   ...
   private void onSTTemplateTreeNodeSelected(STTemplateTreeNode selectedNode) {
      selectedNode.getParentNode(STGroupTreeNode.class)
                  .ifPresent(stGroupTreeNode -> workspace.getSTEditor(stGroupTreeNode.getModel()).setSTTemplate(selectedNode.getModel()));
   }
   ...
   private void onSTInterfaceTreeNodeSelected(STInterfaceTreeNode selectedNode) {
      selectedNode.getParentNode(STGroupTreeNode.class)
                  .ifPresent(stGroupTreeNode -> workspace.getSTEditor(stGroupTreeNode.getModel()).setSTInterface(selectedNode.getModel()));
   }
   ...
   public void onSTModelEditorTreeNodeClicked(nextgen.events.STModelEditorTreeNodeClicked event) {
      ...
      final TreePath path = rootNode
            .find(baseTreeNode -> (baseTreeNode instanceof STTemplateTreeNode) && ((STTemplateTreeNode) baseTreeNode).getModel().equals(stTemplate));
      if (path != null) {
         tree.scrollPathToVisible(path);
         tree.setSelectionPath(path);
      }
   ...
   public void onOpenTemplate(nextgen.events.OpenSTTemplate event) {
      SwingUtilities.invokeLater(() -> {
         final RootNode rootNode = (RootNode) treeModel.getRoot();
         final TreePath path = rootNode
               .find(baseTreeNode -> isSTTemplateTreeNode(baseTreeNode) && ((STTemplateTreeNode) baseTreeNode).getModel().equals(event.model));
         if (path != null) {
            tree.scrollPathToVisible(path);
            tree.setSelectionPath(path);
         }
   ...
   public void onCanvasSTModelClicked(nextgen.events.CanvasSTModelClicked event) {
      appModel().doLaterInTransaction(transaction -> {
         ...
         final RootNode rootNode = (RootNode) treeModel.getRoot();
         final TreePath path = rootNode
               .find(baseTreeNode -> isSTTemplateTreeNode(baseTreeNode) && ((STTemplateTreeNode) baseTreeNode).getModel().equals(stTemplate));
         if (path != null) {
            tree.scrollPathToVisible(path);
            tree.setSelectionPath(path);
         }
