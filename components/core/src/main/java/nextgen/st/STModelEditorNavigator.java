...
public class STModelEditorNavigator extends JPanel {
         ...
         actions.add(newAction("Remove", actionEvent -> {
            presentationModel.doLaterInTransaction(transaction -> {
               if (stArgument instanceof STArgument) {
                  ...
                  argument.getIncomingArgumentsSTModel()
                        .findFirst()
                        .ifPresent(stModel -> stModel.removeArguments(argument));
                  ...
                  argumentKV.getIncomingKeyValuesSTArgument()
                        .findFirst()
                        .ifPresent(stArgument -> stArgument.removeKeyValues(argumentKV));
                  treeModel.removeNodeFromParent(this);
                  editor.setText("", null);
               }
            });
   ...
   public class STValueTreeNode extends BaseTreeNode<nextgen.st.model.STValue> {
      ...
      STValueTreeNode(nextgen.st.model.STValue model, Object stArgument) {
         ...
         this.stArgument = stArgument;
         this.label = getModel().getValue() == null || getModel().getValue()
               .trim()
               .length() == 0 ? "[EMPTY]" : getModel().getValue();
         this.tooltip = "";
         this.uuid = model.getUuid();
   ...
   public class STKVArgumentTreeNode extends BaseTreeNode<STArgument> {
      ...
      protected List<Action> getActions() {
         ...
         actions.add(newAction("Remove", actionEvent -> {
            presentationModel.doLaterInTransaction(transaction -> {
               getModel().getIncomingArgumentsSTModel()
                     .findFirst()
                     .ifPresent(stModel -> stModel.removeArguments(getModel()));
               treeModel.removeNodeFromParent(this);
               editor.setText("", null);
            });
         }));
      ...
      STKVTreeNode(STArgumentKV model, STArgument stArgument, STParameterKey stParameterKey) {
         ...
         if (stValue != null)
            switch (stValue.getType()) {
               case STMODEL:
                  add(new STModelTreeNode(stValue.getStModel(), presentationModel.findSTTemplateByUuid(stValue.getStModel()
                        .getStTemplate()), model));
                  break;
               case PRIMITIVE:
      ...
      STParameterTreeNode(STParameter model, STModel stModel) {
         ...
         stModel.getArgumentsSorted()
               ...
               .forEach(presentationModel.stArgumentConsumer(model)
                     ...
                     .onSingleSTModel((stArgument, stValue) -> add(new STModelTreeNode(stValue.getStModel(), presentationModel
                     ...
                     .onListSTModel((stArgument, stValue) -> add(new STModelTreeNode(stValue.getStModel(), presentationModel
                           .findSTTemplateByUuid(stValue.getStModel().getStTemplate()), stArgument)))
   ...
   public void onSTModelUpdated(STAppEvents.STModelUpdated event) {
      ...
      final BaseTreeNode<?> root = (BaseTreeNode<?>) treeModel.getRoot();

      TreePath path = root.find(baseTreeNode -> baseTreeNode instanceof STModelTreeNode && ((STModelTreeNode) baseTreeNode)
            .getModel()
            .equals(event.stModel));
      if (path == null) return;

      final STModelTreeNode existing = (STModelTreeNode) path.getLastPathComponent();
      existing.nodeChanged();
   }
}