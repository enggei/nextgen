...
public final class NewSTGroup {
	...
	public static void post(nextgen.st.domain.STGroupModel group) {
		log.info("post NewSTGroup");
		org.greenrobot.eventbus.EventBus.getDefault().post(new NewSTGroup(group));
	}

	public final nextgen.st.domain.STGroupModel group;

	public NewSTGroup(nextgen.st.domain.STGroupModel group) {
		this.group = group;
	}
}