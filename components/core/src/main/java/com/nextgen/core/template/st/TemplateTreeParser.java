...
class TemplateTreeParser {
   ...
   private final static org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(TemplateTreeParser.class);

   private final Stack<Integer> expressionStack = new Stack<>();

   TemplateTreeParser(Tree ast, ParameterBuilder parameterBuilder) {
      expressionStack.push(ast.getType());
      for (int i = 0; i < ast.getChildCount(); i++)
         parse(ast.getChild(i), parameterBuilder, i);
   }

   private void parse(Tree ast, ParameterBuilder builder, int childIndex) {
   ...
   public static final class ParameterBuilder {
      ...
      ParameterBuilder(String name, String content) {
         this.name = name;
      ...
      TemplateStatement toStatement() {
         return new TemplateStatement(name, getParameters(), content);
      }
      ...
      ParameterBuilder processMethodCalls(Map<String, ParameterBuilder> map, Map<String, TemplateFile> imports) {
         for (Map.Entry<String, MethodCall> callEntry : methodCalls.entrySet()) {
            for (String methodParam : callEntry.getValue().methodParams) {
               ...
               log.info("'" + this.getName() + "' calls method '" + callEntry.getKey() + "' : parameter '" + myParam.name + "(" + myParam.paramType + ")" + "." + methodParam + ("' ->  set to '" + parameter.getSecond().getType() + "'"));
               myParam.setParamType(parameter.getSecond().getType());
      ...
      private final class Parameter {
         private String name;
         private Template.ParameterType paramType = Template.ParameterType.STRINGPROPERTY;
         private final Set<String> kvNames = new TreeSet<>();

         void setParamType(Template.ParameterType paramType) {
         ...
         void isCollection() {
            this.paramType = Template.ParameterType.LISTPROPERTY;
         }

         void addKvname(String name) {
            this.kvNames.add(name);
            this.paramType = Template.ParameterType.KEYVALUELISTPROPERTY;
         }
